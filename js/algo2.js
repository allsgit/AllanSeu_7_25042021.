...
const ingredientSearch = document.querySelector('.advanced-input-search-ingredient');
const userInputIngredient = ingredientSearch.value.toLowerCase();
const appareilSearch = document.querySelector('.advanced-input-search-appareils');
const userInputappareil = appareilSearch.value.toLowerCase();
const ustensilSearch = document.querySelector('.advanced-input-search-ustensils');
    ...
    if (userInputToLower.length < 1) {
      ...
      for (let i = 0; i < recipes.length; i++) {
        for (let j = 0; j < recipes[i].ingredients.length; j++) {
          if (recipes[i].ingredients[j].ingredient.toLowerCase().includes(userInputToLower)) {
      ...
      if (resultRecipes.length === 0) {
        ...
        recipesCardCreator(searchRecipesWithTag([...setResultRecipes], userInputToLower, tagIngredients, tagUstensils, tagAppareils));
...
function filterBySearch(recipe, keyword) {
  return titleContain(recipe, keyword) || descriptionContain(recipe, keyword) || recipeContainIngredient(recipe, keyword);
}
// .. PAR TITRE
...
function recipeContainIngredient(recipe, keyword) {
  return recipe.ingredients.some((ingredient) => ingredientContain(ingredient, keyword));
}
function titleAndDescriptionContain(recipe, keyword) {
  return titleContain(recipe, keyword) && descriptionContain(recipe, keyword) && recipeContainIngredient(recipe, keyword) && AdvancSearchIngredients(recipes, ingredient);
}
...
function searchRecipesWithTag(recipes, userInputToLower, tagUstensils, tagAppareils) {
  ...
  const recipiesToDisplay = recipes.filter((element) => recipesIngredient.includes(element) && recipesUstensils.includes(element) && recipesAppareils.includes(element));
...
function filterByIngredient(recipes) {
  ...
  const result = [...set].filter((ingredients) => SearchByIngredient(ingredients, userInputIngredient));
...
function filterByAppareil(recipes) {
  ...
  const result1 = [...set].filter((appareil) => searchByAppareil(appareil, userInputappareil));
  ...
  appareilSearch.addEventListener('keyup', () => {
    if (appareilSearch.value.length > 0) {
      ...
      if (userInputappareil == 0) {
        ...
        const result = [...set].filter((appareil) => searchByAppareil(appareil, userInputappareil));
        isMenuOpen = true;
        appareilDiv.innerHTML = ' ';
...
function filterByUstensils(recipes) {
  ...
  const result = [...set].filter((ustensil) => searchByUstensils(ustensil, userInputUstensils));
...
function tagAdd(recipes) {
  ...
  SearchByLi.forEach((tag) => {
    tag.addEventListener('click', (e) => {
      if (tagIngredients.includes(e.target.textContent) || tagAppareils.includes(e.target.textContent) || tagUstensils.includes(e.target.textContent)) {
